# Saia Backend (Rate Quotes + Tracking + Pickup Booking)
...
backend holds the credentials and exposes three endpoints:

- `POST /api/saia/quote` — get a rate quote
- `POST /api/saia/track` — look up live status for a tracking/PRO number
- `POST /api/saia/book-pickup` — schedule a real pickup

## Scope: pickup booking IS available (found 2026-08-14)

Earlier versions of this project concluded pickup booking wasn't possible,
based on Saia's modern developer portal API catalog (which only lists Rate
Quote, Tracking, Bill of Lading, and Imaging). That conclusion was wrong —
Saia has a separate, older "Pickup Web Service" (hosted at saiasecure.com,
confirmed via Saia's own PDF documentation) that genuinely creates real
pickup requests, using the same simple UserID/Password/AccountNumber auth
as Rate Quote and Tracking. See `createPickup()` in `saiaClient.js` for the
full confirmed schema.

**Not yet tested against a real request** — the field names come directly
from Saia's documentation, but as with Rate Quote and Tracking, the first
live call may reveal small surprises even with solid docs in hand. Stays
in mock mode (`SAIA_PICKUP_LIVE_DATA=false`) until you've verified it.

**Important per Saia's own docs:** group all shipments together and only
submit one pickup request per day, per pickup location — don't call this
repeatedly for the same warehouse/day.
