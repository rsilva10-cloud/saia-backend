...
async function getTrackingStatus(input) {
  if (MOCK_MODE) {
    const statuses = CURRENT_STATUS_VALUES;
    const status = statuses[Math.floor(Math.random() * statuses.length)];
    return {
      mock: true,
      trackingNumber: input.trackingNumber,
      status,
      onTime: status === "Delivered" ? "Y" : null,
      latePickup: "N",
      bolNumber: null,
      poNumber: null,
      testMode: "Y",
  ...
  if (code) {
    ...
    // Include the raw XML — code 55 doesn't match any documented Saia error
    // code we've seen, so seeing the actual response is the fastest way to
    // tell whether this is a genuine business error or a SOAP-level fault
    // (e.g. a generic ASP.NET/IIS fault code) unrelated to Saia's own table.
    throw new Error(
  ...
  return {
    mock: false,
    trackingNumber: extractXmlTag(xml, "ProNumber") || input.trackingNumber,
    status: extractXmlTag(xml, "CurrentStatus"),
    onTime: extractXmlTag(xml, "OnTime"),
    latePickup: extractXmlTag(xml, "LatePickup"),
    bolNumber: extractXmlTag(xml, "BLNumber"),
    poNumber: extractXmlTag(xml, "PONumber"),
