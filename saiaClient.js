...
async function getTrackingStatus(input) {
  ...
  const data = await res.json().catch(() => null);
  if (!res.ok) {
    // CONFIRMED real error shape (2026-08-12): { statusCode, message }.
    // Different from Rate Quote's { status: "FAIL", errors: [...] } shape —
    // each Saia product apparently formats errors independently.
    throw new Error(`Saia tracking failed: ${data?.message || JSON.stringify(data)}`);
  }
  return data;
}
