...
/* ---------------------------------------------------------
   PICKUP BOOKING
   CONFIRMED (2026-08-14) from Saia's own PDF documentation for their
   legacy "Pickup Web Service" \u2014 a different, older system (saiasecure.com)
   than the modern developer portal we'd been using for Rate Quote and
   Tracking. This genuinely creates a real pickup request, unlike anything
   in the modern API catalog. Uses the same simple auth pattern as the
   other two: UserID/Password/AccountNumber in the request body, no
   separate subscription key required for this specific service.

   URL:    POST https://www.saiasecure.com/webservice/pickup/xml.aspx
           (plain XML over HTTP/POST \u2014 not SOAP, despite Saia's docs also
           listing a SOAP option at a URL that confusingly says
           ".../shipment/soap.asmx" for the Pickup service; using the
           unambiguous XML/HTTP/POST endpoint instead)
   Body:   <Create> root element (see buildPickupXml below) \u2014 NOT wrapped
           in a SOAP envelope.
   TestMode: Y = sandbox (default here, for safety), N = live. Controlled
           by SAIA_PICKUP_LIVE_DATA.

   IMPORTANT per Saia's own docs: "Group all shipments together and only
   submit 1 pickup request per day for each pickup location." Don't call
   this more than once per warehouse per day.

   STILL UNCONFIRMED: we haven't sent a real request yet, so the exact
   response shape below is built from the documented schema, not a live
   example. Field names should be correct (they're taken directly from
   Saia's own PDF), but real-world quirks may still surface on first use \u2014
   same pattern as Rate Quote and Tracking, where the first real call
   revealed small surprises even with solid docs in hand.
--------------------------------------------------------- */

function pickupLiveMode() {
  return String(process.env.SAIA_PICKUP_LIVE_DATA || "false").toLowerCase() === "true" ? "N" : "Y";
}

// Saia wants HH:MM:SS (8 chars); our UI's time input gives HH:MM.
function toHms(time) {
  if (!time) return "";
  return time.length === 5 ? `${time}:00` : time;
}

function buildPickupXml(input) {
  const detailItems = (input.details || [input]).map((d) => `
    <DetailItem>
      <DestinationZipcode>${d.destinationZip || ""}</DestinationZipcode>
      <DestinationCountry>${d.destinationCountry || "US"}</DestinationCountry>
      <Pieces>${Number(d.pieces) || 0}</Pieces>
      <Package>${d.packageType || "PT"}</Package>
      <Weight>${Number(d.weight) || 0}</Weight>
      <Freezable>${d.freezable ? "Y" : "N"}</Freezable>
      ${d.saiaGuaranteed ? `<SaiaGuaranteed>${d.saiaGuaranteed}</SaiaGuaranteed>` : ""}
    </DetailItem>`).join("");
...
<Create>
  <UserID>${requireEnv("SAIA_USER_ID")}</UserID>
  <Password>${requireEnv("SAIA_PASSWORD")}</Password>
  <TestMode>${pickupLiveMode()}</TestMode>
  ...
  <ContactPhone>${(input.contactPhone || "").replace(/\D/g, "")}</ContactPhone>
  <PickupDate>${input.pickupDate || ""}</PickupDate>
  <ReadyTime>${toHms(input.readyTime)}</ReadyTime>
  <CloseTime>${toHms(input.closeTime)}</CloseTime>
  <SpecialInstructions>${input.specialInstructions || ""}</SpecialInstructions>
  <Details>${detailItems}
  </Details>
</Create>`;
}
...
async function createPickup(input) {
  if (MOCK_MODE) {
    return {
      mock: true,
      pickupNumber: Math.floor(1000000 + Math.random() * 9000000),
      totalPieces: input.pieces || 0,
      totalWeight: input.weight || 0,
      pickupDate: input.pickupDate,
    };
  }

  const url = "https://www.saiasecure.com/webservice/pickup/xml.aspx";
  const body = buildPickupXml(input);

  const res = await fetch(url, {
    method: "POST",
    headers: {
      "Content-Type": "application/xml",
      "Api-Version": "v2", // per Saia's general note about versioned endpoints; harmless if unneeded
    },
    body,
  });
  ...
  const xml = await res.text();
  if (!res.ok) {
    throw new Error(`Saia pickup HTTP error (${res.status}): ${xml}`);
  }
  ...
  if (code) {
    const message = extractXmlTag(xml, "Message");
    const fault = extractXmlTag(xml, "Fault");
    throw new Error(`Saia pickup error (${code}, fault=${fault}): ${message || "no message"} | RAW: ${xml.slice(0, 2000)}`);
  }

  return {
    mock: false,
    pickupNumber: extractXmlTag(xml, "PickupNumber"),
    totalPieces: extractXmlTag(xml, "TotalPieces"),
    totalWeight: extractXmlTag(xml, "TotalWeight"),
    nextBusinessDate: extractXmlTag(xml, "NextBusinessDate"),
    testMode: extractXmlTag(xml, "TestMode"),
    raw: xml,
  };
}
